...
namespace KBSpecs_test
{
    public partial class Form14 : KryptonForm
    {
        ...
        private void kryptonButton1_Click(object sender, EventArgs e)
        {
            ...
            foreach (PaletteButtonSpecStyle type in Enum.GetValues(typeof(PaletteButtonSpecStyle)))
            {
                ...
                foreach (PaletteButtonStyle style in Enum.GetValues(typeof(PaletteButtonStyle)))
                {
                    ...
                    string sizestyle = $"{kryptonRibbon1.Height - 89}";
                    Bitmap buttonImage = new Bitmap(70, 50);
                    Graphics g = Graphics.FromImage(buttonImage);
                    g.CopyFromScreen(new Point(Left + 1122, Top + 32), new Point(0, 0), buttonImage.Size);
        ...
        private async void kryptonButton3_Click(object sender, EventArgs e)
        {
            ...
            var buttonImage = new Bitmap(400, 180);
            Graphics g = Graphics.FromImage(buttonImage);
            SendKeys.Send("%(f)");
            await Task.Delay(300);
            Application.DoEvents();
            g.CopyFromScreen(new Point(Left , Top ), new Point(0, 0), buttonImage.Size);
            Clipboard.SetImage(buttonImage);
            Size = new Size(1208, 512);
        }
        ...
        {
            s_control = "KRibbonFileAppButton";
            s_idControl = 14;
            MessageBox.Show($"Style count: {Enum.GetValues(typeof(PaletteButtonStyle)).Length} Type count: {Enum.GetValues(typeof(PaletteButtonSpecStyle)).Length}");
            ...
            foreach (PaletteButtonSpecStyle type in Enum.GetValues(typeof(PaletteButtonSpecStyle)))
            {
                buttonSpecAppMenu1.Type = type;
                if (type == PaletteButtonSpecStyle.Generic)
                {
                    buttonSpecAppMenu1.Image = Properties.Resources._2;
                }
                else
                {
                    buttonSpecAppMenu1.Image = null;
                }

                Image i_image = buttonSpecAppMenu1.GetImage(KryptonManager.CurrentGlobalPalette, PaletteState.Normal) ?? new Bitmap(16, 16);
                Clipboard.SetImage(i_image);
                string size = $"{i_image.Width}x{i_image.Height}";
                s_idType += 1;
                //MessageBox.Show($"Image type: {type}, size: {size}");
                ...
                foreach (PaletteButtonStyle style in Enum.GetValues(typeof(PaletteButtonStyle)))
                {
                    buttonSpecAppMenu1.Style = style;
                    //MessageBox.Show($"clipboard set, type: {type}, style: {style}");
                    MessageBoxAutoClose.Show("Sucess", 50);
                    string sizestyle = "";//$"{kryptonRibbon1.Height}";
                    Bitmap buttonImage = new Bitmap(70, 50);
                    Graphics g = Graphics.FromImage(buttonImage);
                    ...
                    await Task.Delay(300);
                    Application.DoEvents();
                    g.CopyFromScreen(new Point(Left + 202, Top + 111), new Point(0, 0), buttonImage.Size);
                    Clipboard.SetImage(buttonImage);
