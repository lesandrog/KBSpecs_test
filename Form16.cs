...
namespace KBSpecs_test
{
    public partial class Form16 : KryptonForm
    {
        ...
        {
            s_control = "KPage KDockableNavigator";
            s_idControl = 16;
            ...
            buttonSpecNavigator1.Image = null;
            buttonSpecNavigator1.TypeRestricted = Krypton.Navigator.PaletteNavButtonSpecStyle.Generic;
            buttonSpecNavigator1.Style = PaletteButtonStyle.Inherit;
            ...
            foreach (PaletteButtonSpecStyle type in Enum.GetValues(typeof(PaletteButtonSpecStyle)))
            {
                ...
                foreach (PaletteButtonStyle style in Enum.GetValues(typeof(PaletteButtonStyle)))
                {
                    ...
                    string sizestyle = $"{kryptonDockableNavigator1.Height - kryptonPanel1.Height - 5}";
                    Bitmap buttonImage = new Bitmap(70, 50);
                    Graphics g = Graphics.FromImage(buttonImage);
                    g.CopyFromScreen(new Point(Left + 120, Top + 45), new Point(0, 0), buttonImage.Size);
        ...
        {
            var buttonImage = new Bitmap(400, 136);
            Graphics g = Graphics.FromImage(buttonImage);
        ...
        {
            s_control = "KDockableNavigator";
            s_idControl = 26;
            MessageBox.Show($"Style count: {Enum.GetValues(typeof(PaletteButtonStyle)).Length} TypeRestricted count: {Enum.GetValues(typeof(PaletteNavButtonSpecStyle)).Length}");
            ...
            buttonSpecAny1.Image = null;
            buttonSpecAny1.Type = PaletteButtonSpecStyle.Generic;
            buttonSpecAny1.Style = PaletteButtonStyle.Inherit;


            foreach (PaletteNavButtonSpecStyle TypeRestricted in Enum.GetValues(typeof(PaletteNavButtonSpecStyle)))
            {
                buttonSpecNavigator1.TypeRestricted = TypeRestricted;
                if (TypeRestricted == PaletteNavButtonSpecStyle.Generic)
                {
                    buttonSpecNavigator1.Image = Properties.Resources._2;
                }
                else
                {
                    buttonSpecNavigator1.Image = null;
                ...
                Image i_image = buttonSpecNavigator1.GetImage(KryptonManager.CurrentGlobalPalette, PaletteState.Normal) ?? new Bitmap(16, 16);
                Clipboard.SetImage(i_image);
                string size = $"{i_image.Width}x{i_image.Height}";
                s_idType += 1;
                //MessageBox.Show($"Image type: {type}, size: {size}");
                ...
                foreach (PaletteButtonStyle style in Enum.GetValues(typeof(PaletteButtonStyle)))
                {
                    buttonSpecNavigator1.Style = style;
                    //MessageBox.Show($"clipboard set, type: {type}, style: {style}");
                    MessageBoxAutoClose.Show("Sucess", 50);
                    string sizestyle = $"{kryptonDockableNavigator1.Height - kryptonPanel1.Height - 2}";
                    Bitmap buttonImage = new Bitmap(70, 50);
                    Graphics g = Graphics.FromImage(buttonImage);
                    g.CopyFromScreen(new Point(Left + 340, Top + 45), new Point(0, 0), buttonImage.Size);
                    Clipboard.SetImage(buttonImage);
                ...
                dataTable1.Rows.Add(s_idControl, s_control, s_idType, $"{TypeRestricted}", i_image, size,
                                    i_Inherit, s_Inherit,
                                    i_Standalone, s_Standalone,
                                    i_Alternate, s_Alternate,
                                    i_LowProfile, s_LowProfile,
